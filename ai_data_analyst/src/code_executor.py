...
# Add the current directory to sys.path to ensure we can import rag_agent
...
try:
    ...
    def rag_search(query: str) -> str:
        ...
        Returns:
            The answer generated by the RAG agent based on retrieved documents.
        """
        try:
            inputs = {"question": query, "retry_count": 0}
            # Use invoke to run the RAG pipeline
            result = rag_agent.app.invoke(inputs)
            return result.get("generation", "No response generated.")
        except Exception as e:
            return f"Error performing RAG search: {str(e)}"
            
except Exception as e:
    print(f"Warning: Could not import rag_agent: {e}")
    def rag_search(query: str) -> str:
        return "RAG search is currently unavailable."
...
def _collect_tables(values: Iterable[Any], original_df: Optional[pd.DataFrame]) -> List[pd.DataFrame]:
...
def execute_code(code: str, dataframe: Optional[pd.DataFrame] = None, db_path: Optional[str] = None) -> ExecutionResult:
    """Execute LLM-provided Python code inside a restricted namespace."""
    if not code.strip():
        return ExecutionResult(error="No code to execute.")

    # --- Security Guard: block __import__ ---
    if "__import__" in code:
        return ExecutionResult(
            error="Direct use of __import__ is not allowed."
        )

    # --- Sanitize: Comment out standard import statements ---
    # LLMs often include imports by habit. We strip them to allow execution 
    # using the pre-loaded environment.
    code_lines = code.split('\n')
    sanitized_lines = []
    for line in code_lines:
        # Check if line starts with import/from (ignoring whitespace)
        # We only care about valid python statements, so case-sensitive 'import'
        stripped = line.strip()
        if stripped.startswith("import ") or stripped.startswith("from "):
            sanitized_lines.append(f"# {line}  # [Auto-commented by security sandbox]")
        else:
    ...
    # Convert db_path to string with forward slashes for cross-platform compatibility
    if db_path:
        # Ensure db_path is a string and use forward slashes
    ...
    safe_globals: Dict[str, Any] = {
        ...
        "df": dataframe.copy() if dataframe is not None else None,
        
        # Database path (if available) - converted to string with forward slashes
        "db_path": db_path_str,
    ...
    # If we are in DB mode, we might want to provide a helper or just let them use sqlite3.connect(db_path)
    if db_path:
        # We can pre-create a connection if we want, but letting them connect is fine too.
        # Just make sure they know the variable name 'db_path'.
        pass
    ...
    try:
        with redirect_stdout(stdout_buffer):
            ...
            if is_expression and len(code_lines) > 1:
                ...
                # Debug logging
                print(f"Executing code with db_path: {db_path_str}")
                # print(f"Code:\n{code}") 
        ...
        return ExecutionResult(
            ...
            error=f"Database Error: {str(e)}\nHint: Ensure you are using the 'db_path' variable and not a hardcoded path. db_path is: {db_path_str}"
        )
    except Exception:
        return ExecutionResult(
            stdout=stdout_buffer.getvalue(),
